    ...
    public function update(Request $request, $id)
    {
        try {
            if(isset($request->form_img)){
                ...
                if (isset($mediaItem[0])) {
                    $mediaItem[0]->name = $request->media_title;
                    // Spatie MediaLibrary doesn't have a built-in 'alt' column; store it as custom property
                    $mediaItem[0]->setCustomProperty('alt', $request->media_alt);
                    $mediaItem[0]->save();
                }
                if(isset($mediaItem[0])){
                ...
                $mediaItem[0]->setCustomProperty('alt', $request->alt);
                $mediaItem[0]->save();
                return view('admin.media.show',compact('media'));
            }
        } catch (Throwable $th) {
